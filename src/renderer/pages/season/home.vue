<template>
  <div class="d-flex" id="home-wrapper">
    <h1 class="font-weight-black ml-12 mt-12" v-if="!isPlayoffs">
      Week {{ currentWeek + 1 }}
    </h1>
    <h1 class="font-weight-black ml-12 mt-12" v-else>Playoffs</h1>
    <img
      id="season-logo"
      src="../../assets/img/fedexcup_logo.png"
      alt="FedEx Cup Logo"
    />
    <VideoPlayer v-if="!watchedVideo && isPlayoffs"></VideoPlayer>
    <div class="season-info">
      <div class="ml-11" id="upcoming-banner">
        <v-card flat dark ripple width="100%" color="#083666">
          <v-img
            class="white--text align-end"
            height="250px"
            :src="
              isPlayoffs
                ? require('../../assets/img/northern_trust.jpg')
                : schedule[currentWeek].picture
                ? schedule[currentWeek].picture
                : require('@/assets/img/generic_course.jpg')
            "
          ...
          <v-img
            ...
            :src="
              currentWeek === 1
                ? require('../../assets/img/bmw_championship.jpg')
                : require('../../assets/img/tour_championship.jpg')
            "
            v-else
          >
          </v-img>
          <v-card-title class="mb-n8 text-uppercase" v-if="!activeGame">
            <span v-if="isPlayoffs && !isEndOfSeason"
              >Upcoming Playoff Tournament</span
            >
          ...
          <v-card-title class="mb-n8 text-uppercase" v-else>
            <span v-if="isPlayoffs && !isEndOfSeason"
              >Continue Playoff Tournament</span
            >
            <span v-else-if="!isEndOfSeason">Continue Tournament</span>
          </v-card-title>
          <v-card-title>
            <span v-if="isPlayoffs && !isEndOfSeason">
              {{ playoffs[currentWeek].name }}
            </span>
            <span v-else-if="!isEndOfSeason">
          ...
          <v-card-subtitle>
            <span v-if="isPlayoffs && !isEndOfSeason">
              {{ playoffs[currentWeek].course }}
            </span>
            <span v-else-if="!isEndOfSeason">
              {{ schedule[currentWeek].course }}
            </span>
    ...
    <div id="fedex-leaderboard">
      <div class="leaderboard-header d-flex">
        <h1
          class="headline text-uppercase font-weight-bold ml-5 mt-4 white--text"
        >
          FedExcup Season Standings
        </h1>
        <v-spacer></v-spacer>
        <v-btn
          text
          class="white--text align-self-center"
          to="/season/leaderboard"
          >View Full Leaderboard</v-btn
        >
      </div>
      <v-data-table
        height="580px"
    ...
    <v-card dark tile class="ticker d-flex">
      ...
      <v-card-subtitle>
        <MarqueeText
          :duration="schedule.length * 2.75"
          :repeat="schedule.length < 10 ? 10 : null"
          v-if="!isPlayoffs"
        >
          <span
            v-for="event in schedule.filter(event => !event.winner)"
            :key="event.id"
          >
        ...
        <MarqueeText :repeat="10" v-else>
          <span
            v-for="event in playoffs.filter(event => !event.winner)"
            :key="event.id"
          >
            {{ event.name }} - {{ event.course }} &nbsp; &nbsp; | &nbsp; &nbsp;
          </span>
