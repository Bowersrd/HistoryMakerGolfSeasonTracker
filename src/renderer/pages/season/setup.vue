      ...
      <v-stepper v-model="e1">
        <v-stepper-header>
          <v-stepper-step color="#083666" :complete="e1 > 1" step="1"
            >Set Season Roster</v-stepper-step
          >
          <v-divider></v-divider>
          <v-stepper-step color="#083666" :complete="e1 > 2" step="2"
            >Set Season Schedule</v-stepper-step
          >
          <v-divider></v-divider>
        ...
        <v-stepper-items>
          ...
          <v-stepper-content step="2">
            <div class="event-container">
              ...
              <h4 class="ml-8 my-3" v-show="createdSchedule.length === 0">
                No events added yet for Regular Season!
              </h4>
              <v-card
                width="98%"
                color="#E4E4E4"
              ...
              >
                <v-row class="d-flex align-center">
                  <v-col cols="8">
                    ...
                    <v-card-subtitle>
                      {{ event.course }} • Purse: ${{
                        new Intl.NumberFormat("en-US").format(event.purse)
                      }}
                    </v-card-subtitle>
                  </v-col>
                  <v-col cols="2">
                  ...
                  <v-col cols="2">
                    <v-icon class="mr-4 mt-3" @click="moveUp(event)">
                      fas fa-sort-up
                    </v-icon>
                    <v-icon class="mr-2 mb-2" @click="moveDown(event)">
                      fas fa-sort-down
                    </v-icon>
                    <v-icon small class="mx-2" @click="editItem(event)">
                      fas fa-edit
                    </v-icon>
                    <v-icon small @click="deleteItem(event)">
                      fas fa-trash-alt
                    </v-icon>
                  </v-col>
                </v-row>
              ...
              >
                <v-row class="d-flex align-center">
                  <v-col cols="10">
                    ...
                    <v-card-subtitle>
                      {{ playoff.course }} • Purse: ${{
                        new Intl.NumberFormat("en-US").format(playoff.purse)
                      }}
                    </v-card-subtitle>
                  </v-col>
                  <v-col cols="2">
            ...
            <v-dialog v-model="dialog" max-width="500px">
              <template v-slot:activator="{ on }">
                <div class="d-flex" height="500px">
                  <v-btn color="#CC0001" dark class="mt-4" v-on="on"
                    >Add Event</v-btn
                  >
            ...
            <v-btn dark color="#083666" class="mt-3" text @click="e1 = 1">
              Back
            </v-btn>
          </v-stepper-content>
          <v-stepper-content step="3">
            <div class="review-container">
              <div class="event-container" id="review">
                <h3 class="ml-5 font-weight-bold">
                  Season Roster ({{ rosters.length }})
                </h3>
                <h4 class="ml-8 my-3" v-show="rosters.length === 0">
                  No players added to the roster!
                </h4>
                <v-card
                  height="80px"
                  color="#E4E4E4"
                ...
                >
                  ...
                  <v-card-subtitle>
                    {{ roster.country }} | Tournament Grade: {{ roster.grade }}
                  </v-card-subtitle>
                </v-card>
              </div>
              ...
              <div class="event-container" id="review">
                <h3 class="ml-5 font-weight-bold">
                  Regular Season ({{ createdSchedule.length }})
                </h3>
                <h4 class="ml-8 my-3" v-show="createdSchedule.length === 0">
                  No events added yet for Regular Season!
                </h4>
            ...
            <v-btn dark color="#083666" class="mt-3" text @click="e1 = 2">
              Back
            </v-btn>
          </v-stepper-content>
        </v-stepper-items>
