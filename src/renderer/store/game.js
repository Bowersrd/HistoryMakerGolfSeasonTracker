...
export const mutations = {
  ...
  CONVERT_SCORES: state => {
    state.field.forEach(player => {
      ...
      let tempScore =
        state.convertedScores[state.tournamentScore.score].scores[0];
      ...
      switch (player.score) {
        ...
        case "B":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[1];
          player.score = tempScore + grade;
          break;
        case "B-":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[2];
          player.score = tempScore + grade;
          break;
        case "C+":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[3];
          player.score = tempScore + grade;
          break;
        case "C":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[4];
          player.score = tempScore + grade;
          break;
        case "C-":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[5];
          player.score = tempScore + grade;
          break;
        case "D+":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[6];
          player.score = tempScore + grade * 2;
          break;
        case "D":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[7];
          player.score = tempScore + grade * 2;
          break;
        case "D-":
          tempScore = player.score =
            state.convertedScores[state.tournamentScore.score].scores[8];
          player.score = tempScore + grade * 2;
          break;
        case "CUT":
          player.score = 500;
  ...
  SETUP_LEADERBOARD: (state, { par, pairs }) => {
    if (
      state.pairings[0].a.scorecard[0].score !== null ||
      state.currentHole !== 1
    ) {
  ...
  ADD_SCORE: (state, { score, letter, today, coursePar, pairs }) => {
    ...
    if (letter === "a") {
      state.pairings[state.group].a.scorecard[
        state.currentHole - 1
      ].score = score;
      ...
      state.pairings[state.group].a.total =
        state.pairings[state.group].a.score - coursePar + todayA;

      // state.pairings[state.group].a.scorecard.forEach(hole => hole.score = 4) -- USED FOR DEBUGGING
    } else {
      state.pairings[state.group].b.scorecard[
        state.currentHole - 1
      ].score = score;
      ...
      state.pairings[state.group].b.total =
        state.pairings[state.group].b.score - coursePar + todayB;
  ...
  NEXT_HOLE: state => {
    if (state.counter >= 108) {
      const totalScores = state.pairings
        .map(pair => pair.a.total)
        .concat(state.pairings.map(pair => pair.b.total));
      const lowestScore = Math.min(...totalScores);
      const tied =
        totalScores.filter(score => score === lowestScore).length > 1;
  ...
  RANK_FIELD: state => {
    ...
    field.forEach(player => {
      if (
        field.indexOf(player) !== 0 &&
        player.score === field[field.indexOf(player) - 1].score
      ) {
        player.rank = "";
        rank++;
      } else {
  ...
  ADD_PERK: (state, { perk, letter }) => {
    ...
    if (letter === "a") {
      if (playerA.perks.map(perk => perk.name).includes(newPerk.name)) {
        let exsistingPerk = playerA.perks.find(
          perk => perk.name === newPerk.name
        );
      ...
      if (playerB.perks.map(perk => perk.name).includes(newPerk.name)) {
        let exsistingPerk = playerB.perks.find(
          perk => perk.name === newPerk.name
        );
  ...
  ADD_TFN: (state, { perk, letter }) => {
    ...
    letter === "a"
      ? (editPerkA.tfn = !editPerkA.tfn)
      : (editPerkB.tfn = !editPerkB.tfn);
  ...
  SET_WINNER: (state, id) => {
    ...
    state.field.forEach(player =>
      player.id === id ? (player.score = player.score - 1) : ""
    );
